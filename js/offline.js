...
//   • waitForServiceWorker() — gates app init until SW controls page
// ─────────────────────────────────────────────────────────────
...
/**
 * Waits until the Service Worker is controlling this page before
 * returning. This prevents main.js from attempting Firebase/network
 * calls before the SW cache is in place — the root cause of cold
 * offline start failures.
 *
 * Behaviour:
 *   • Already controlled  → resolves immediately (common case after
 *                           first install; all subsequent visits).
 *   • Not yet controlled  → waits for 'controllerchange' event, which
 *                           fires when the SW calls clients.claim().
 *   • Timeout (4 s)       → resolves anyway so the app never hangs
 *                           in environments where SW is unavailable.
 *   • No SW support       → resolves immediately (graceful fallback).
 *
 * Usage in main.js:
 *   import { waitForServiceWorker } from './offline.js';
 *   await waitForServiceWorker();
 *   // safe to initialize Firebase / auth here
 */
export function waitForServiceWorker() {
  // No SW support (e.g. old browser, non-HTTPS) — proceed normally
  if (!('serviceWorker' in navigator)) {
    return Promise.resolve();
  }

  // SW is already controlling this page — nothing to wait for.
  // This is the common case on every visit after the first install.
  if (navigator.serviceWorker.controller) {
    console.log('[offline] SW already controlling page ✓');
    return Promise.resolve();
  }

  console.log('[offline] Waiting for SW to claim page…');

  // Race: controllerchange fires when SW calls clients.claim()
  // Timeout: 4 s safety net so the app never hangs indefinitely
  return Promise.race([
    new Promise((resolve) => {
      navigator.serviceWorker.addEventListener(
        'controllerchange',
        () => {
          console.log('[offline] SW claimed page ✓');
          resolve();
        },
        { once: true }
      );
    ...
    new Promise((resolve) => {
      setTimeout(() => {
        console.warn('[offline] SW claim timeout — proceeding without SW control.');
        resolve();
      }, 4000);
    }),
  ]);
